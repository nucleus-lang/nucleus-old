...
struct Parser {
	...
	static std::unique_ptr<AST::Expression> ParseParenthesis() {
		...
		if (Lexer::CurrentToken != ')') return LogError("Expected ')'. Current Token: " + std::to_string(Lexer::CurrentToken) + ".");
	...
	static std::unique_ptr<AST::Expression> ParseIdentifier(std::string currId = "") {
		...
		if (Lexer::CurrentToken != ')') {
			while (1) {
				...
				if (Lexer::CurrentToken != Token::TK_Comma) return LogError("Expected ')' or ',' in argument list");
	...
	static std::unique_ptr<AST::Expression> ParseBinaryOperatorRight(int ExpressionPrecedence, std::unique_ptr<AST::Expression> LHS) {
		while (1) {
			...
			if (Lexer::CurrentToken == '}') return LHS;
		...
		if (Lexer::CurrentToken == Token::TK_Identifier) {
			...
			if (Lexer::CurrentToken == Token::TK_Number) {
				...
				if (Lexer::NumValString.find(".") != std::string::npos)
					 NumValResult = Lexer::NumValString.back() == 'f' ? int(std::stof(Lexer::NumValString)) : int(std::stod(Lexer::NumValString));
	...
	static std::unique_ptr<AST::Expression> ParseForExpression() {
		...
		Lexer::GetNextToken();
		if (Lexer::CurrentToken != Token::TK_Identifier) return LogError("Expected identifier after for. Current Token: " + std::to_string(Lexer::CurrentToken) + ".");
		...
		Lexer::GetNextToken();
		if (Lexer::CurrentToken != '{') return LogError("Expected '{' in for loop.");
	...
	static std::unique_ptr<AST::Expression> ParseUnary() {
		...
		if (auto Operand = ParseUnary()) return std::make_unique<AST::Unary>(Opc, std::move(Operand));
		return nullptr;
	}
		...
		while (1) {
			...
			if (Lexer::CurrentToken == '=') {
				...
				if (dynamic_cast<AST::ArrayInitContent*>(Init.get()) != nullptr) {
					...
					if (getAIC == nullptr) return LogError("ArrayInitContent is nullptr.");
					...
					if (getNAC == nullptr) return LogError("NestedArrayContent is nullptr.");
		...
		while (Lexer::CurrentToken != tokenEnd) {
			...
			switch (Lexer::CurrentToken) {
				...
				case Token::TK_Define:
					...
					if (auto FnAST = Parser::ParseDefinition()) PushBackParsedFunction(std::move(FnAST));
...
void PushBackParsedFunction(std::unique_ptr<AST::Function> FnAST) { ParseTesting::allParsedFunctions.push_back(std::move(FnAST)); }
