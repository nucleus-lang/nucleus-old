...
llvm::Value* AST::Number::codegen() {
	...
	if (isDouble) {
		...
		switch (bit) {
			case 1:   getType = llvm::IntegerType::getInt1Ty(*CodeGeneration::TheContext); break;
			case 8:   getType = llvm::IntegerType::getInt8Ty(*CodeGeneration::TheContext); break;
			case 16:  getType = llvm::IntegerType::getInt16Ty(*CodeGeneration::TheContext); break;
			case 32:  getType = llvm::IntegerType::getInt32Ty(*CodeGeneration::TheContext); break;
			case 64:  getType = llvm::IntegerType::getInt64Ty(*CodeGeneration::TheContext); break;
			case 128: getType = llvm::IntegerType::getInt128Ty(*CodeGeneration::TheContext); break;
			default:  getType = llvm::IntegerType::getInt32Ty(*CodeGeneration::TheContext); break;
		}
	...
	if (isArrayElement || isArrayPointer) {
		...
		if (isArrayElement && !vType->isStructTy()) {
			...
			if (isArrayPointer || structMemberStore) {
				...
				if (varType == nullptr) return CodeGeneration::LogErrorV("Struct Member not found :(");
...
llvm::Function* AST::FunctionPrototype::codegen() {
	...
	if (type == nullptr) return CodeGeneration::LogErrorFLLVM("type is nullptr.");

	if (dynamic_cast<Double*>(type.get()) != nullptr) FT = llvm::FunctionType::get(llvm::Type::getDoubleTy(*CodeGeneration::TheContext), llvmArgs, false);
	...
	else if (dynamic_cast<Float*>(type.get()) != nullptr)   FT = llvm::FunctionType::get(llvm::Type::getFloatTy(*CodeGeneration::TheContext), llvmArgs, false);
	else if (dynamic_cast<Generic*>(type.get()) != nullptr) FT = llvm::FunctionType::get(llvm::PointerType::getUnqual(*CodeGeneration::TheContext), llvmArgs, false);
...
llvm::Function* AST::Function::codegen() {
	...
	for (auto& A : TheFunction->args()) {
		...
		CodeGeneration::DBuilder->insertDeclare(Alloca, D, CodeGeneration::DBuilder->createExpression(), llvm::DILocation::get(SP->getContext(), LineNo, 0, SP), CodeGeneration::Builder->GetInsertBlock());
	...
	if (P.IsBinaryOperator()) Parser::BinaryOpPrecedence.erase(prototype->GetOperatorName());
...
llvm::Value* AST::If::codegen() {
	...
	if (isInt) ConditionV = CodeGeneration::Builder->CreateICmpNE(ConditionV,  llvm::ConstantInt::get(ConditionV->getType(), 0), "ifcond");
...
llvm::Value* AST::Var::codegen() {
	...
	for (unsigned i = 0, e = VarNames.size(); i != e; ++i) {
		...
		if (dynamic_cast<NestedArray*>(VarNames[i].type.get()) == nullptr) CodeGeneration::Builder->CreateStore(InitVal, Alloca);
...
llvm::Value* AST::NestedArray::codegen() {
	...
	std::vector<llvm::Constant*> values;
	if (arrays.size() == 0) return CodeGeneration::LogErrorV("Array size is 0.");
...
llvm::Type* AST::GetASTIntegerType(AST::Integer* getI) {
	switch (getI->bit) {
		case 1:   return llvm::Type::getInt1Ty(*CodeGeneration::TheContext);
		case 8:   return llvm::Type::getInt8Ty(*CodeGeneration::TheContext);
		case 16:  return llvm::Type::getInt16Ty(*CodeGeneration::TheContext);
		case 32:  return llvm::Type::getInt32Ty(*CodeGeneration::TheContext);
		case 64:  return llvm::Type::getInt64Ty(*CodeGeneration::TheContext);
		case 128: return llvm::Type::getInt128Ty(*CodeGeneration::TheContext);
	}
