...
llvm::Value* AST::Number::codegen() {
	...
	if (isDouble) {
		...
		if (bit == 1)        getType = llvm::IntegerType::getInt1Ty(*CodeGeneration::TheContext);
		else if (bit == 8)   getType = llvm::IntegerType::getInt8Ty(*CodeGeneration::TheContext);
		else if (bit == 16)  getType = llvm::IntegerType::getInt16Ty(*CodeGeneration::TheContext);
		else if (bit == 32)  getType = llvm::IntegerType::getInt32Ty(*CodeGeneration::TheContext);
		else if (bit == 64)  getType = llvm::IntegerType::getInt64Ty(*CodeGeneration::TheContext);
		else if (bit == 128) getType = llvm::IntegerType::getInt128Ty(*CodeGeneration::TheContext);
		else                 getType = llvm::IntegerType::getInt32Ty(*CodeGeneration::TheContext);
...
llvm::Value* AST::Variable::codegen() {
	...
	if (!V) CodeGeneration::LogErrorV("Unknown variable name: " + name + "\n");
...
//
	...
	if (isArrayElement || isArrayPointer) {
		...
		if (isArrayElement && !vType->isStructTy()) {
			...
			if (!vType->isPointerTy()) return CodeGeneration::Builder->CreateLoad(V->getAllocatedType()->getArrayElementType(), CodeGeneration::Builder->CreateInBoundsGEP(vType, V, llvm::ArrayRef<llvm::Value*>(indexList, 2), idx.c_str()), name.c_str());
			else {
				indexList[0] = index;
				indexList[1] = nullptr;
			...
			if (isArrayPointer || structMemberStore) {
				if (!vType->isPointerTy()) return CodeGeneration::Builder->CreateInBoundsGEP(vType, V, llvm::ArrayRef<llvm::Value*>(indexList, 2), name.c_str());
				else {
					indexList[0] = index;
					indexList[1] = nullptr;
				...
				if (llvm::ConstantInt* CI = static_cast<llvm::ConstantInt*>(index)) {
					if (CI->getBitWidth() <= 32) structMemberIndex = CI->getSExtValue();
				}
				...
				for (int i = 0; i < Parser::AllStructs.size(); i++) {
					if (Parser::AllStructs[i]->Name == std::string(structVar->getName())) {
						...
						if (dynamic_cast<Double*>(tExpr) != nullptr) varType = llvm::Type::getDoubleTy(*CodeGeneration::TheContext);
						else if (dynamic_cast<Integer*>(tExpr) != nullptr) { Integer* getI = (Integer*)tExpr; varType = GetASTIntegerType(getI); }
						else if (dynamic_cast<Float*>(tExpr) != nullptr) varType = llvm::Type::getFloatTy(*CodeGeneration::TheContext);
		...
		} else finalV = CodeGeneration::Builder->CreateInBoundsGEP(vType, V, llvm::ArrayRef<llvm::Value*>(indexList, 2), idx.c_str());
	...
	CodeGeneration::isPureNumber = isInt;
...
llvm::Value* GetLessThanOperator(llvm::Value* L, llvm::Value* R, int pCount, AST::Integer* LInt) {
	...
	if (isDouble) {
		...
		if (!LInt) return CodeGeneration::LogErrorV("Invalid Variable Type. Expected Integer.");
...
llvm::Value* AST::Binary::codegen() {
	...
	if (CodeGeneration::isPureNumber) pureIntCount++;
	...
	if (op == '=') {
		if (secondOp == '\0') {
			...
			if (!LHSE) return CodeGeneration::LogErrorV("Destination of '=' must be a variable.");
			...
			if (!Variable) return CodeGeneration::LogErrorV("Unknown variable name: " + LHSE->name + ".");

			CodeGeneration::Builder->CreateStore(RVal, Variable);
			return RVal;
		} else if (secondOp == '=') opLLVM = GetEqualsOperator(L, R, pureIntCount, (AST::Integer*)lhs.get());
	}

	if (!L || !R) return nullptr;
	//std::cout << "Warning: One of the Values is nullptr.\n";

	if (op == Token::TK_DotComma || op == ';')  return R;
	else if (op == '+') opLLVM = pureIntCount == 2 ? CodeGeneration::Builder->CreateNUWAdd(L, R, "addtmp") : CodeGeneration::Builder->CreateFAdd(L, R, "addtmp");
	else if (op == '-') opLLVM = pureIntCount == 2 ? CodeGeneration::Builder->CreateNUWSub(L, R, "subtmp") : CodeGeneration::Builder->CreateFSub(L, R, "subtmp");
	else if (op == '*') opLLVM = pureIntCount == 2 ? CodeGeneration::Builder->CreateNUWMul(L, R, "multmp") : CodeGeneration::Builder->CreateFMul(L, R, "multmp");
	else if (op == '<') opLLVM = GetLessThanOperator(L, R, pureIntCount, (AST::Integer*)lhs.get());
	else if (op == '>') opLLVM = GetGreaterThanOperator(L, R, pureIntCount, (AST::Integer*)lhs.get());
	...
	if (opLLVM == nullptr) {
		...
		if (F == nullptr) return CodeGeneration::LogErrorV("Invalid binary operator (" + std::to_string(op) + ").\n");
...
llvm::Value* AST::Call::codegen() {
	...
	if (!CalleeF) return CodeGeneration::LogErrorV("Unknown function " + callee + " referenced.\n");
...
llvm::Function* AST::FunctionPrototype::codegen() {
	...
	for (auto const& i: arguments) {
		if      (dynamic_cast<   Double*    >(i.first.get()) != nullptr) llvmArgs.push_back(llvm::Type::getDoubleTy(*CodeGeneration::TheContext));
		else if (dynamic_cast<   Integer*   >(i.first.get()) != nullptr) { Integer* getI = (Integer*)i.first.get(); llvmArgs.push_back(GetASTIntegerType(getI)); }
	...
	else return CodeGeneration::LogErrorFLLVM("Unknown function type.");

	if (FT == nullptr) return CodeGeneration::LogErrorFLLVM("FT is nullptr.");
...
llvm::Function* AST::Function::codegen() {
	...
	llvm::DIFile* Unit = CodeGeneration::DBuilder->createFile(CodeGeneration::DebugInfo::TheCU->getFilename(), CodeGeneration::DebugInfo::TheCU->getDirectory());
	...
	for (auto& A : TheFunction->args()) {
		...
		llvm::DILocalVariable* D = CodeGeneration::DBuilder->createParameterVariable(SP, A.getName(), ArgIdx, Unit, LineNo, AST::GetFunctionDIType(P.arguments[ArgIdx].first.get()), true);
...
llvm::Value* AST::If::codegen() {
	...
	if (isInt) {	
		ConditionV = CodeGeneration::Builder->CreateICmpNE(ConditionV, llvm::ConstantInt::get(ConditionV->getType(), 0), "ifcond");
	} else if (isDouble || isFloat) {
		//std::cout << "Condition is double or float!\n";
	
		ConditionV = CodeGeneration::Builder->CreateFCmpONE(ConditionV, llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0)), "ifcond");
	} else return CodeGeneration::LogErrorV("Unknown condition type in 'if' statement");
	...
	} else ElseV = llvm::ConstantInt::get(TheFunction->getFunctionType()->getReturnType(), 0);
...
llvm::Value* AST::For::codegen() {
	...
	*CurVar = CodeGeneration::Builder->CreateLoad(Alloca->getAllocatedType(), Alloca, varName.c_str());
	...
	if (Step) {
		...
		if (dynamic_cast<Double*>(varType.get()) != nullptr || dynamic_cast<Float*>(varType.get()) != nullptr) NextVar = CodeGeneration::Builder->CreateFAdd(CurVar, StepVal, "nextvar");		
		else if (dynamic_cast<Integer*>(varType.get()) != nullptr) NextVar = CodeGeneration::Builder->CreateNUWAdd(CurVar, StepVal, "nextvar");
		else return CodeGeneration::LogErrorV("Unknown function type.");
	} else {
	...
	if (isDouble || isFloat) {
		...
		EndCond = CodeGeneration::Builder->CreateFCmpONE(EndCond, llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0)), "loopcond");
		...
		if (ty == nullptr) return CodeGeneration::LogErrorV("Function Type returned nullptr.");

		EndCond = CodeGeneration::Builder->CreateICmpNE(EndCond, llvm::ConstantInt::get(EndCond->getType(), 0), "loopcond");
	...
	if (oldValue) CodeGeneration::NamedValues[varName] = oldValue;
	else CodeGeneration::NamedValues.erase(varName);

	if (Next != nullptr) {
		llvm::Value* NextVar = Next->codegen();
		return !NextVar ? nullptr : NextVar;
	} else {
		...
		if (dynamic_cast<Integer*>(varType.get()) != nullptr) {
			Integer* getI = (Integer*)varType.get();
			return llvm::Constant::getNullValue(GetASTIntegerType(getI));
		}

		if (dynamic_cast<Float*>(varType.get()) != nullptr)
...
llvm::Value* AST::Unary::codegen() {
	...
	if (OpCode == '-') {
		...
		if (OperandV->getType()->isFloatTy() || OperandV->getType()->isDoubleTy())
	...
	if (!F) return CodeGeneration::LogErrorV(std::string("Unknown unary operator: ") + OpCode);
...
llvm::Value* AST::Var::codegen() {
	...
	for (unsigned i = 0, e = VarNames.size(); i != e; ++i) {
		...
		if (Init) {
			...
			} else InitVal = Init->codegen();
			if (!InitVal) return nullptr;

			//std::cout << "CodeGen Init Successfull!" << std::endl;
		} else {
			if (dynamic_cast<Double*>(VarNames[i].type.get()))     InitVal = llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0));
			else if (dynamic_cast<Float*>(VarNames[i].type.get())) InitVal = llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0f));
			...
			else return CodeGeneration::LogErrorV("Unknown variable type.");
		}
		...
		if (dynamic_cast<NestedArray*>(VarNames[i].type.get()) == nullptr) {
			Alloca = CodeGeneration::Builder->CreateAlloca(!VarNames[i].type->isPointer ? getT : getT->getPointerTo(), 0, VarName.c_str());
		} else Alloca = (llvm::AllocaInst*)InitVal;
...
llvm::Value* AST::Array::codegen() {
	...
	if (dynamic_cast<Double*>(type.get()) != nullptr)       getT = llvm::Type::getDoubleTy(*CodeGeneration::TheContext);
	else if (dynamic_cast<Integer*>(type.get()) != nullptr) getT = GetASTIntegerType((Integer*)type.get());
	else if (dynamic_cast<Float*>(type.get()) != nullptr)   getT = llvm::Type::getFloatTy(*CodeGeneration::TheContext);
	else return CodeGeneration::LogErrorV("Unknown function type.");
	...
	if (variables.size() == 0 || variables.size() != size) {
		for (unsigned int i = 0; i < size; i++) {
			if (dynamic_cast<Double*>(type.get()) != nullptr)     values.push_back(llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0)));
			else if (dynamic_cast<Float*>(type.get()) != nullptr) values.push_back(llvm::ConstantFP::get(*CodeGeneration::TheContext, llvm::APFloat(0.0f)));
			else if (dynamic_cast<Integer*>(type.get()) != nullptr) {
		...
		for (unsigned int i = 0; i < size; i++) {
			...
			if (static_cast<llvm::ConstantFP*>(getV) != nullptr)          values.push_back((llvm::ConstantFP*)getV);
			else if (static_cast<llvm::ConstantInt*>(getV) != nullptr)    values.push_back((llvm::ConstantInt*)getV);
			else if (static_cast<llvm::ConstantStruct*>(getV) != nullptr) values.push_back((llvm::ConstantStruct*)getV);
			else return CodeGeneration::LogErrorV("Variable initialized in array is not constant.");
		}
	}
...
llvm::Value* AST::ArrayInitContent::codegen() {
	...
	if (dynamic_cast<Number*>(variables[0].get()) != nullptr) {
		...
		if (num->isDouble) getT = llvm::Type::getDoubleTy(*CodeGeneration::TheContext);
		else if (num->isInt) {
			auto i = std::make_unique<Integer>(0);
			i->bit = num->bit;

			getT = GetASTIntegerType(i.get());
		} else if (num->isFloat) getT = llvm::Type::getFloatTy(*CodeGeneration::TheContext);
		else return CodeGeneration::LogErrorV("Unknown function type.");
...
llvm::Value* AST::NestedArray::codegen() {
	...
	for (unsigned int i = 0; i < arrays.size(); i++) {
		...
		llvm::Value* getPtr = CodeGeneration::Builder->CreateInBoundsGEP(Alloca->getAllocatedType(), Alloca, llvm::ArrayRef<llvm::Value*>(indexList, 2), "TDArrayPtr");
...
void AST::EmitLocation(AST::Expression* a) {
	if (!a)return CodeGeneration::Builder->SetCurrentDebugLocation(llvm::DebugLoc());
...
llvm::DIType* AST::GetFunctionDIType(AST::Expression* t) {
	if (dynamic_cast<AST::Double*>(t) != nullptr) return CodeGeneration::DebugInfo::getDoubleTy();
	if (dynamic_cast<AST::Integer*>(t) != nullptr) return CodeGeneration::DebugInfo::getIntegerTy();
	if (dynamic_cast<AST::Float*>(t) != nullptr) return CodeGeneration::DebugInfo::getFloatTy();
...
llvm::DISubroutineType* AST::CreateFunctionType(unsigned NumArgs, AST::Expression* t) {
	...
	for (unsigned i = 0, e = NumArgs; i != e; ++i) EltTys.push_back(Typ);
...
llvm::Type* AST::GetASTIntegerType(AST::Integer* getI) {
	if (getI->bit == 1)        return llvm::Type::getInt1Ty(*CodeGeneration::TheContext);
	else if (getI->bit == 8)   return llvm::Type::getInt8Ty(*CodeGeneration::TheContext);
	else if (getI->bit == 16)  return llvm::Type::getInt16Ty(*CodeGeneration::TheContext);
	else if (getI->bit == 32)  return llvm::Type::getInt32Ty(*CodeGeneration::TheContext);
	else if (getI->bit == 64)  return llvm::Type::getInt64Ty(*CodeGeneration::TheContext);
	else if (getI->bit == 128) return llvm::Type::getInt128Ty(*CodeGeneration::TheContext);
